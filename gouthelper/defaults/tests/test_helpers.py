...
class TestDefaultsGetGoalUrate(TestCase):
    def setUp(self):
        ...
        self.userless_goalurate = GoalUrateFactory(goal_urate=GoalUrates.FIVE, ultaid=self.userless_ultaid)

    def test__returns_goal_urate_from_object_with_GoalUrate(self):
        """Method that returns the goal_urate from a GoalUrate object."""
        goalurate = defaults_get_goalurate(goal_urate_object=self.userless_ultaid)
