...
class TestGetMedHistorytypeAids(TestCase):
    def setUp(self):
        ...
        self.patient_ultaid = create_ultaid(patient=self.patient)
        self.patient_mhtype_aids = MedHistoryTypesAids(ULTAID_MEDHISTORYS, self.patient)
    ...
    def test__init__sets_mhtypes(self):
        self.assertEqual(self.patient_mhtype_aids.mhtypes, ULTAID_MEDHISTORYS)
        self.assertEqual(self.flare_mhtype_aids.mhtypes, FLARE_MEDHISTORYS)

    def test__init__sets_related_object(self):
        self.assertEqual(self.patient_mhtype_aids.related_object, self.patient)
    ...
    def test__init__sets_dict_of_aid_tuple_of_model_and_medhistorytypes(self):
        self.assertEqual(
            self.patient_mhtype_aids.dict_of_aid_tuple_of_model_and_medhistorytypes,
    ...
    def test__init_sets_aid_attr_and_aid_medhistorys(self):
        self.assertEqual(self.patient_mhtype_aids.UltAid, UltAid)
        self.assertEqual(self.patient_mhtype_aids.ULTAID_MEDHISTORYS, ULTAID_MEDHISTORYS)
    ...
    def test__mhtype_in_related_objects_medhistorys(self) -> None:
        for mhtype in ULTAID_MEDHISTORYS:
            self.assertTrue(self.patient_mhtype_aids.mhtype_in_related_objects_medhistorys(mhtype, UltAid))
        for mhtype in PPX_MEDHISTORYS:
            if mhtype not in ULTAID_MEDHISTORYS:
                self.assertFalse(self.patient_mhtype_aids.mhtype_in_related_objects_medhistorys(mhtype, UltAid))
    ...
    def test__related_object_is_or_has_aid_type(self) -> None:
        self.assertTrue(self.patient_mhtype_aids.related_object_is_or_has_aid_type("ultaid", UltAid))
        self.assertFalse(self.patient_mhtype_aids.related_object_is_or_has_aid_type("flare", Flare))
        ...
        patient_ultaid_mhtype_aids = MedHistoryTypesAids(ULTAID_MEDHISTORYS, self.patient_ultaid)
        with self.assertRaises(ValueError):
            patient_ultaid_mhtype_aids.related_object_is_or_has_aid_type("ultaid", UltAid)
    ...
    def test__related_object_error_check(self) -> None:
        self.assertIsNone(self.patient_mhtype_aids.related_object_error_check())
        self.assertIsNone(self.flare_mhtype_aids.related_object_error_check())
        patient_ultaid_mhtype_aids = MedHistoryTypesAids(ULTAID_MEDHISTORYS, self.patient_ultaid)
        with self.assertRaises(ValueError):
            patient_ultaid_mhtype_aids.related_object_error_check()
        no_related_object_mhtype_aids = MedHistoryTypesAids(ULTAID_MEDHISTORYS)
        with self.assertRaises(ValueError):
    ...
    def test__mhtype_in_aid_medhistorys(self) -> None:
        for mhtype in ULTAID_MEDHISTORYS:
            self.assertTrue(self.patient_mhtype_aids.mhtype_in_aid_medhistorys(mhtype, "ultaid"))
        for mhtype in PPX_MEDHISTORYS:
            if mhtype not in ULTAID_MEDHISTORYS:
                self.assertFalse(self.patient_mhtype_aids.mhtype_in_aid_medhistorys(mhtype, "ultaid"))
        for mhtype in FLARE_MEDHISTORYS:
    ...
    def test__related_object_is_or_has_flare(self) -> None:
        ...
        self.assertFalse(self.patient_mhtype_aids.related_object_is_or_has_flare)
        create_flare(patient=self.patient)
        patient_with_flare_qs = Patient.objects.flares_qs().filter(pk=self.patient.pk).first()
        patient_with_flare_qs_mhtype_aids = MedHistoryTypesAids(FLARE_MEDHISTORYS, patient_with_flare_qs)
        self.assertTrue(patient_with_flare_qs_mhtype_aids.related_object_is_or_has_flare)

    def test__mhtype_in_related_object_aid(self) -> None:
        for mhtype in ULTAID_MEDHISTORYS:
            self.assertTrue(self.patient_mhtype_aids.mhtype_in_related_object_aid(mhtype))
        for mhtype in PPX_MEDHISTORYS:
            if mhtype not in ULTAID_MEDHISTORYS:
                self.assertFalse(self.patient_mhtype_aids.mhtype_in_related_object_aid(mhtype))
        for mhtype in FLARE_MEDHISTORYS:
