...
class TestGoalUrateRules(TestCase):
    def setUp(self):
        ...
        self.prov_psp_gu = create_goalurate(patient=self.prov_psp)
        self.admin_psp_gu = create_goalurate(patient=self.admin_psp)
        self.anon_psp_gu = create_goalurate(patient=self.anon_psp)
        self.anon_gu = create_goalurate()

    def test__add_object(self):
        """Test that a Provider or Admin can add GoalUrates for Patient's for whom they are the provider
        and that anyone can add an anonymous Patient's GoalUrate or an anonymous GoalUrate."""
    ...
    def test__view_object(self):
        """Test that a Provider or Admin can view GoalUrates for Patient's for whom they are the provider
        and that anyone can view an anonymous Patient's GoalUrate or an anonymous GoalUrate."""
    ...
    def test__change_object(self):
        """Test that a Provider or Admin can change GoalUrates for Patient's for whom they are the provider
        and that anyone can change an anonymous Patient's GoalUrate or an anonymous GoalUrate."""
