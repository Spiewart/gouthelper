...
from ...ppxaids.selectors import ppxaid_patient_qs, ppxaid_patientless_qs
from ...ppxaids.tests.factories import create_ppxaid
from ...treatments.choices import NsaidChoices, SteroidChoices, Treatments
from ..helpers import get_medallergy, get_medallergys
...
class TestMedAllergyAttr(TestCase):
    """Test the get_medallergy helper function"""
    ...
    def setUp(self):
        ...
        self.patient_ppxaid = create_ppxaid(mas=self.trt_mas, patient=True)
        self.patient = self.patient_ppxaid.patient
        self.empty_ppxaid = create_ppxaid(mas=[])
        self.empty_patient_ppxaid = create_ppxaid(mas=[], patient=True)
        self.empty_patient = self.empty_patient_ppxaid.patient

    def test__medallergys_qs_ppxaid(self):
        """Test the method when the objects have a medallergys_qs attribute"""
        ppxaid = ppxaid_patientless_qs(pk=self.ppxaid.pk).get()
        self.assertTrue(hasattr(ppxaid, "medallergys_qs"))
        with self.assertNumQueries(0):
            for trt in self.trt_mas:
                if trt in NsaidChoices.values:
                    self.assertTrue(get_medallergy(trt, ppxaid))
    ...
    def test__medallergys_qs_empty_ppxaid(self):
        empty_ppxaid = ppxaid_patientless_qs(pk=self.empty_ppxaid.pk).get()
        with self.assertNumQueries(0):
            for trt in self.trt_mas:
                if trt in NsaidChoices.values:
    ...
    def test__medallergys_qs_pseudopatient(self):
        pseudpatient = ppxaid_patient_qs(pseudopatient=self.patient.pk).get()
        with self.assertNumQueries(0):
            for trt in self.trt_mas:
                if trt in NsaidChoices.values:
    ...
    def test__medallergys_qs_empty_pseudopatient(self):
        empty_pseudopatient = ppxaid_patient_qs(pseudopatient=self.empty_patient.pk).get()
        with self.assertNumQueries(0):
            for trt in self.trt_mas:
                if trt in NsaidChoices.values:
    ...
    def test__without_medallergys_qs(self):
        ...
        for trt in self.trt_mas:
            if trt in NsaidChoices.values:
                ...
                self.assertTrue(self.patient_ppxaid.nsaid_allergy)
                self.assertTrue(self.patient.nsaid_allergy)
                self.assertIn(trt, [ma.treatment for ma in self.ppxaid.nsaid_allergy])
                self.assertIn(trt, [ma.treatment for ma in self.patient_ppxaid.nsaid_allergy])
                self.assertIn(trt, [ma.treatment for ma in self.patient.nsaid_allergy])
                self.assertFalse(self.empty_ppxaid.nsaid_allergy)
                self.assertFalse(self.empty_patient_ppxaid.nsaid_allergy)
                self.assertFalse(self.empty_patient.nsaid_allergy)
            else:
                self.assertTrue(getattr(self.ppxaid, f"{trt.lower()}_allergy"))
                self.assertTrue(getattr(self.patient_ppxaid, f"{trt.lower()}_allergy"))
                self.assertTrue(getattr(self.patient, f"{trt.lower()}_allergy"))
                self.assertFalse(getattr(self.empty_ppxaid, f"{trt.lower()}_allergy"))
                self.assertFalse(getattr(self.empty_patient_ppxaid, f"{trt.lower()}_allergy"))
                self.assertFalse(getattr(self.empty_patient, f"{trt.lower()}_allergy"))


class TestMedAllergysGet(TestCase):
    """Test the get_medallergys helper function"""
    ...
    def test__allopurinol(self):
        self.assertEqual(
            get_medallergys(self.medallergys, Treatments.ALLOPURINOL), getattr(self, Treatments.ALLOPURINOL.lower())
        )
        self.assertIsNone(get_medallergys(self.empty_medallergys, Treatments.ALLOPURINOL))

    def test__colchicine(self):
        self.assertEqual(
            get_medallergys(self.medallergys, Treatments.COLCHICINE), getattr(self, Treatments.COLCHICINE.lower())
        )
        self.assertIsNone(get_medallergys(self.empty_medallergys, Treatments.COLCHICINE))

    def test__febuxostat(self):
        self.assertEqual(
            get_medallergys(self.medallergys, Treatments.FEBUXOSTAT), getattr(self, Treatments.FEBUXOSTAT.lower())
        )
        self.assertIsNone(get_medallergys(self.empty_medallergys, Treatments.FEBUXOSTAT))

    def test__nsaids(self):
        nsaid_allergy = get_medallergys(self.medallergys, NsaidChoices.values)
        for nsaid in NsaidChoices:
            self.assertIn(getattr(self, nsaid.lower()), nsaid_allergy)
        self.assertFalse(get_medallergys(self.empty_medallergys, NsaidChoices.values))

    def test__probenecid(self):
        self.assertEqual(
            get_medallergys(self.medallergys, Treatments.PROBENECID), getattr(self, Treatments.PROBENECID.lower())
        )
        self.assertIsNone(get_medallergys(self.empty_medallergys, Treatments.PROBENECID))

    def test__steroids(self):
        steroid_allergy = get_medallergys(self.medallergys, SteroidChoices.values)
        ...
        self.assertFalse(get_medallergys(self.empty_medallergys, SteroidChoices.values))
