...
class FlareAidAbout(TemplateView):
    ...
    @property
    def page(self):
        return apps.get_model("pages.Page").objects.get(slug="about", context=Contexts.FLAREAID, tag=None)
...
class FlareAidDetail(DetailView):
    ...
    def get_object(self, *args, **kwargs) -> FlareAid:
        ...
        if flareaid.uptodate is False:
            flareaid.update()
        return flareaid

    @property
    def pages(self):
        return apps.get_model("pages.Page").objects.filter(context=Contexts.FLAREAID)
